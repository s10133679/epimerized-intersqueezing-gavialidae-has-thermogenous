...
public class HeadHunterTest extends TestCase {
	HeadHunter testNormal, testNormal2,testNormal3, testZero, testNegative, testHuge;
	Image testImage;
	SurvivalMap map;
	SurvivalGame game;
	
	public void setUp(){
		testNormal = new HeadHunter(5,10,Direction.LEFT,1);
		testNormal2 = new HeadHunter(15,20,Direction.LEFT,1);
		testNormal3 = new HeadHunter(50,50,Direction.LEFT,1);
		testZero = new HeadHunter(0,0,Direction.LEFT,2);
		testNegative = new HeadHunter(-5,-10,Direction.LEFT,3);
		testHuge = new HeadHunter(5000,10000,Direction.LEFT,4);
		
		testImage = new ImageIcon("SURVIVAL/evil.png").getImage();
		
		map = new SurvivalMap(50,50);
	...
	public void testGetSetImageImage() {
		...
		assertEquals(testNormal.getImage(), new ImageIcon("SURVIVAL/hero.png").getImage());
	...
	public void testGetSetImageString() {
		assertEquals(testNormal.getImage(), new ImageIcon("SURVIVAL/evil.png").getImage());
		
		//SET NEW IMAGE
		testNormal.setImage("SURVIVAL/evil.png");
		assertEquals(testNormal.getImage(), new ImageIcon("SURVIVAL/evil.png").getImage());
	}
	...
	public void testOnEventDieToZombie() {
		//spawn a mouse and a hero at (8,9)... call onEvent(). game should exit
		HeadHunter tempHunter = new HeadHunter(8,9,Direction.LEFT,1);
		game.getMap().addMappable(tempHunter);
		...
		assertEquals(tempMappable.contains(tempHunter), true); //there is a mouse and a hero, just added one above;
		assertEquals(tempMappable.contains(tempHero), true);
		
		tempHero.onEvent(new GameEvent("HeadHunterMovement", game)); //on event... there should be no hero now
